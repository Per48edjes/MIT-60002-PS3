...
class RectangularRoom(object):
    ...
    def __init__(self, width, height, dirt_amount):
        ...
        # Create dict that contains all tiles set with dirt_amount to start
    ...
    def get_num_cleaned_tiles(self):
        ...
        """
        clean_tiles = 0
        for _, tile_dirt in self.tiles.items():
            if tile_dirt == 0:
                clean_tiles += 1
    ...
    def is_position_in_room(self, pos):
        ...
        """
        pos_x, pos_y = math.floor(pos.get_x()), math.floor(pos.get_y())
        return (pos_x, pos_y) in self.tiles
...
class StandardRobot(Robot):
    ...
    def update_position_and_clean(self):
        ...
        if self.room.is_position_valid(new_pos):
            ...
            # Clean as much as capacity will allow in current position
            self.room.clean_tile_at_position(self.get_robot_position(), self.capacity)
        else:
            # If not a valid position, change direction randomly
...
def run_simulation(num_robots, speed, capacity, width, height, dirt_amount, min_coverage, num_trials,
    ...
    for trial_no in range(num_trials):
        ...
        for robot_no in range(num_robots):
            robot_crate[robot_no] = robot_type(room, speed, capacity)

        # Keep the robots running until min_coverage achieved
        time_step = 0
        current_coverage = room.get_num_cleaned_tiles() / room.get_num_tiles()
        while current_coverage < min_coverage:
            # Update each robot 1 time step
            for _, robot in robot_crate.items():
                robot.update_position_and_clean()
            # Record the time_step
            time_step += 1
            # Check how much has been cleaned
            current_coverage = room.get_num_cleaned_tiles() / room.get_num_tiles()

        # Add data to trial_times
